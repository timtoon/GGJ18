...
public class CreateAndWinLose : MonoBehaviour 
{
	public static CreateAndWinLose instance;
	...
	public void InputDataUpdateFrequency(float frequency)
	{
		...
		for (int x = 0; x < Cells.Count; x++)
		{
			...
			CellStateController myCell = C.GetComponent<CellStateController>();
	...
	public void SetDie(GameObject DeadCell)
	{
		...
		if (DeadCell.GetComponent<CellStateController>().isEvil)
		{
			print("Bad Died");
			BadCells--;
			if (BadCells <= 0)
    ...
    private GameObject CreateGoodCell()
    {
		...
		//var c = new Cell();
		myCell.tag = "Player";
		CellStateController c = myCell.GetComponent<CellStateController>();
        c.isEvil = false;
